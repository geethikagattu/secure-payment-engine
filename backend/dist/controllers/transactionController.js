...
exports.createTransaction = async (req, res) => {
    ...
    try {
        ...
        // Create transaction
        const [transactionResult] = await connection.query('INSERT INTO transactions (user_id, product_id, amount, status, payment_method, ip_address, fraud_score) VALUES (?, ?, ?, ?, ?, ?, ?)', [userId, productId, product.price, status, paymentMethod, ipAddress, fraudScore]);
        // Update user balance only if not flagged
        if (status === 'success') {
            await connection.query('UPDATE users SET balance = balance - ? WHERE id = ?', [product.price, userId]);
        }
...
exports.getTransactionHistory = async (req, res) => {
    try {
        const userId = req.user.id;
        const [transactions] = await db.query(`SELECT t.*, p.name as product_name, p.category 
       FROM transactions t 
